...
const EMPTY_PROD = { name: "", description: "", unitPrice: 0, unit: "ay", categoryId: "", sku: "", order: 0 };
const EMPTY_CAT = { name: "", description: "", order: 0 };
const HOSTING_FILTER = /hosting|server|vps|sunucu|wartung|infrastruktur/i;
  ...
  return (
    <div data-testid="hosting-packages-page">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-extrabold text-[#0f172a]">Hosting ve Sunucu Paketleri</h1>
          <p className="text-[#64748b] mt-1">Bu sayfadan sadece hosting, sunucu ve altyapı paketlerini yönetebilirsiniz. Paketler profesyonel metinlerle müşteriye sunulacak şekilde düzenlenebilir.</p>
        </div>
        <button
        ...
        >
          <Plus size={15} /> Yeni Hosting Paketi
        </button>
      </div>

      <div className="flex gap-2 mt-5">
        <button onClick={() => setEditingProd({ ...EMPTY_PROD })} className="px-4 py-2 rounded-lg text-sm font-bold inline-flex items-center gap-2 bg-[#0f172a] text-white"><Package size={14} /> Yeni Hosting Paketi</button>
        <button onClick={() => setEditingCat({ ...EMPTY_CAT })} className="px-4 py-2 rounded-lg text-sm font-bold inline-flex items-center gap-2 bg-slate-100 text-[#0f172a]"><Server size={14} /> Yeni Kategori</button>
      </div>
      ...
      <div className="mt-10 bg-white rounded-2xl shadow-card p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-[#0f172a]">Hosting Kategorileri</h2>
            <p className="text-[#64748b] mt-1">Hosting ve sunucu paketlerini daha kolay yönetmek için kategori oluşturun veya düzenleyin.</p>
          </div>
        ...
        <div className="mt-6">
          {hostingCats.length === 0 ? (
            <div className="p-8 text-center text-[#64748b]">Henüz hosting kategorisi bulunmuyor. Sağ üstten yeni kategori ekleyebilirsiniz.</div>
          ) : (
              ...
              {hostingCats.map((c) => (
                <li key={c.id} className="rounded-3xl border border-slate-200 p-4 bg-slate-50 flex flex-col justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-[#64748b] mt-1">{c.description || "Açıklama yok."}</p>
                  </div>
      ...
      {editingProd && (
        <Modal title={editingProd.id ? "Paket düzenle" : "Yeni Paket"} onClose={() => setEditingProd(null)}>
          <Field label="Paket adı *"><input value={editingProd.name} onChange={(e) => setEditingProd({ ...editingProd, name: e.target.value })} data-testid="product-name" className={inp} /></Field>
          <Field label="Açıklama"><textarea value={editingProd.description || ""} onChange={(e) => setEditingProd({ ...editingProd, description: e.target.value })} rows={3} className={inp} /></Field>
          <div className="grid grid-cols-3 gap-3">
            <Field label="Fiyat (CHF) *"><input type="number" step="0.01" value={editingProd.unitPrice} onChange={(e) => setEditingProd({ ...editingProd, unitPrice: e.target.value })} data-testid="product-price" className={inp} /></Field>
            <Field label="Birim"><input value={editingProd.unit} onChange={(e) => setEditingProd({ ...editingProd, unit: e.target.value })} className={inp} /></Field>
      ...
      {editingCat && (
        <Modal title={editingCat.id ? "Kategori düzenle" : "Yeni Kategori"} onClose={() => setEditingCat(null)}>
          <Field label="Kategori adı *"><input value={editingCat.name} onChange={(e) => setEditingCat({ ...editingCat, name: e.target.value })} data-testid="cat-name" className={inp} /></Field>
          <Field label="Açıklama"><textarea value={editingCat.description || ""} onChange={(e) => setEditingCat({ ...editingCat, description: e.target.value })} rows={2} className={inp} /></Field>
          <div className="flex justify-end gap-2 pt-3 border-t border-slate-200">

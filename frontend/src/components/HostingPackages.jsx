          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-3">
            {DOMAIN_AUCTIONS.map((auction) => (
              <article key={auction.domain} className="rounded-[34px] border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-black p-6 shadow-2xl">
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  <button onClick={() => setBidModal(auction)} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold hover:bg-white/10">Gebot abgeben</button>
        ...
        {bidModal && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setBidModal(null)}>
            <motion.div className="relative max-w-lg w-full mx-4 rounded-[32px] bg-slate-950 p-7 text-white border border-white/10 shadow-2xl" initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }} onClick={(e) => e.stopPropagation()}>
              ...
              <input type="number" defaultValue={bidModal.currentBid + 50} className="mt-2 w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-4 text-2xl font-black outline-none focus:border-emerald-400" />
              ...
              <button onClick={() => navigate("/register")} className="mt-5 w-full rounded-2xl bg-emerald-500 px-5 py-4 font-black text-white hover:bg-emerald-600">Registrieren und Gebot bestätigen</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>

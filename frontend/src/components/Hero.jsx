...
export default function Hero() {
  ...
  return (
    <section id="top" className="relative min-h-screen bg-[#020617] overflow-hidden flex items-center justify-center">
      <MatrixBg />
      <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-transparent to-black/60 pointer-events-none" />

      {/* Support Badge */}
      {s.badgeEnabled && (
        <div className="hidden sm:block absolute top-24 right-4 md:right-10 z-20">
          <div className="relative">
            <div className="absolute inset-0 bg-[#E63946] rounded-full blur-2xl opacity-30 animate-pulse" />
            <div className="relative w-32 h-32 md:w-44 md:h-44 lg:w-52 lg:h-52 rounded-full bg-gradient-to-br from-[#E63946] to-[#a8202e] border-2 border-white/20 flex flex-col items-center justify-center text-center text-white px-3 md:px-5 shadow-2xl hover:scale-105 transition-transform duration-500">
      ...
      <div className="relative z-10 text-center px-4 sm:px-6 max-w-5xl pt-24 sm:pt-0">
        <div className="min-h-[100px] sm:min-h-[120px] md:min-h-[160px] flex items-center justify-center">
          <h1 key={idx} className="fade-in text-[32px] sm:text-[40px] md:text-[68px] font-extrabold tracking-tight text-balance">
            <span className="text-[#1E88E5]">{slide.highlight}</span>
            <span className="text-white"> {slide.word}</span>
          </h1>
        </div>

        <p className="text-white/85 text-[16px] md:text-[20px] mt-4 leading-relaxed max-w-3xl mx-auto">
          {renderRich(s.heroSubtitle)}
        </p>
        {s.heroTagline && (
          <p className="text-[#FFC107] font-semibold text-[15px] md:text-[18px] mt-2">
            {s.heroTagline}
          </p>
        )}

        <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center items-center">
          <button onClick={openContact} className="btn-cta btn-red">
            <span className="text-[11px] font-normal opacity-90">{s.btnContactSmall}</span>
            <span className="text-[17px]">{s.btnContactLarge}</span>
          </button>
          <button onClick={() => openQuote()} className="btn-cta btn-blue">
            <span className="text-[11px] font-normal opacity-90">{s.btnQuoteSmall}</span>
            <span className="text-[17px]">{s.btnQuoteLarge}</span>
          </button>
        </div>
      </div>

      <div
        className="absolute bottom-16 left-1/2 -translate-x-1/2 z-10 text-white/70 bouncing-arrow cursor-pointer"
        onClick={() => document.getElementById("stats")?.scrollIntoView({ behavior: "smooth" })}
      >
        <ChevronsDown size={36} />
      </div>

      <div className="absolute bottom-0 left-0 right-0 bg-black/90 py-3 px-6 z-10">
        <div className="max-w-[1400px] mx-auto flex flex-wrap items-center justify-between gap-4 text-white text-xs">
          <div className="flex items-center gap-4 flex-wrap">
            {(s.partners || []).map((p, i) => (
              <span key={i} className="font-bold text-[10px] tracking-wider opacity-80">{p}</span>
            ))}
          </div>
          {s.ratingText && (
